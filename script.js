...
function criarLabel(tipo, textoPersonalizado = null) {
    ...
    span.textContent =
    textoPersonalizado || tipo.charAt(0).toUpperCase() + tipo.slice(1);
    return span;
}

function processText(text) {
    if (!text) return '';
    // troca qualquer combinação de \n ou \r por <br>
    return text.replace(/(\r\n|\n|\r)+/g, '<br>').trim();
}
...
function findPrincipio(estrutura, artigoNode) {
    ...
    for (const node of nodes) {
        ...
        if (node.filhos.includes(artigoNode)) {
            return node.nome;
        }
        }
        if (node.filhos) {
...
function criarNode(node, parentNode = null) {
    ...
    artigoDiv.addEventListener('click', () => {
        ...
        if (node.conteudo) {
        conteudoModal += `<p>${processText(node.conteudo)}</p>`;
        }
